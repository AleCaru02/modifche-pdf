...
async function loadFile(file) {
  ...
  try {
    ...
    await render();
    updateButtons();
    toast('PDF caricato. Tocca una scritta oppure usa “Modifica area”.');
...
async function renderTextHits(page, viewport, spec) {
  ...
  if (count === 0 && !spec.items.some((item) => item.kind === 'replace')) {
    note.textContent = 'Questa pagina sembra una scansione. Usa “Modifica area”; OCR resta disponibile solo quando lo richiedi.';
    note.classList.remove('hide');
    clearTimeout(note.timer);
    note.timer = setTimeout(() => note.classList.add('hide'), 5500);
...
function openTextEditor(meta) {
  ...
  modal.innerHTML = `<div class="text-editor-card" role="dialog" aria-modal="true" aria-label="Modifica testo PDF"><div class="editor-title">Modifica testo o numero</div><div class="editor-note">Il contenuto vecchio viene coperto con un piccolo margine di sicurezza per evitare residui sotto la cifra.</div><textarea class="editor-input" rows="3"></textarea><div class="editor-size-row"><label>Dimensione <input class="editor-size" type="number" min="5" max="80" step="0.5"></label></div><label class="editor-check"><input type="checkbox" class="keep-width" checked> Mantieni esattamente la larghezza dell’area</label><div class="editor-actions"><button type="button" class="cancel">Annulla</button><button type="button" class="save primary">Applica modifica</button></div></div>`;
  document.body.appendChild(modal);
  const input = modal.querySelector('.editor-input');
  const sizeInput = modal.querySelector('.editor-size');
  input.value = model.text || '';
  sizeInput.value = Number(model.fontSize || 16).toFixed(1);
  input.style.font = cssFont(model, 18);
  modal.querySelector('.cancel').onclick = () => modal.remove();
  modal.onclick = (event) => { if (event.target === modal) modal.remove(); };
  modal.querySelector('.save').onclick = () => {
    const value = input.value;
    const nextSize = clamp(parseFloat(sizeInput.value) || model.fontSize || 16, 5, 80);
    if (existing) {
      snapshot();
      existing.text = value;
      existing.fontSize = nextSize;
      existing.keepWidth = modal.querySelector('.keep-width').checked;
      selectedId = existing.id;
    } else {
      snapshot();
      const item = { id: makeId(), kind: 'replace', originalId: meta.originalId, originalText: meta.text, text: value, x: meta.x, y: meta.y, w: Math.max(meta.w, 0.2), h: Math.max(meta.h, 0.2), fontSize: nextSize, fontFamily: meta.fontFamily, fontWeight: meta.fontWeight, fontStyle: meta.fontStyle, angle: meta.angle || 0, color: meta.color, background: meta.background, keepWidth: modal.querySelector('.keep-width').checked };
  ...
  input.addEventListener('keydown', (event) => {
    ...
    if ((event.ctrlKey || event.metaKey) && event.key === 'Enter') modal.querySelector('.save').click();
  });
  requestAnimationFrame(() => { input.focus(); input.select(); });
}

function replaceScreenArea(rect, initialText = '') {
  if (!pdf || !canvas.width) return false;
  const bounds = canvas.getBoundingClientRect();
  const left = clamp(rect.left, 0, bounds.width);
  const top = clamp(rect.top, 0, bounds.height);
  const width = clamp(rect.width, 4, bounds.width - left);
  const height = clamp(rect.height, 4, bounds.height - top);
  const scaleX = canvas.width / Math.max(bounds.width, 1);
  const scaleY = canvas.height / Math.max(bounds.height, 1);
  const colors = sampleTextColors(left * scaleX, top * scaleY, width * scaleX, height * scaleY);
  const estimatedFont = clamp((height / (BASE_SCALE * zoom)) * 0.82, 5, 80);
  openTextEditor({
    originalId: `manual:${makeId()}`,
    text: initialText,
    x: pct(left, bounds.width),
    y: pct(top, bounds.height),
    w: pct(width, bounds.width),
    h: pct(height, bounds.height),
...
function addItem(kind, x = 16, y = 16, extra = {}) {
  snapshot();
  const item = { id: makeId(), kind, x, y, w: kind === 'text' ? 34 : kind === 'line' ? 25 : 24, h: kind === 'text' ? 5 : kind === 'line' ? 0.35 : 8, text: 'Scrivi qui', fontSize: 16, fontFamily: 'Arial, Helvetica, sans-serif', fontWeight: 'normal', fontStyle: 'normal', color: '#1769e0', background: '#ffffff', angle: 0, ...extra };
  pageSpec().items.push(item);
  selectedId = item.id;
  setTool('select');
  render();
...
sheet.onpointerdown = (event) => {
  ...
  if (['white', 'highlight', 'rect', 'line'].includes(tool)) {
    ...
    const move = (next) => {
      ...
      item.x = dx < 0 ? x + dx : x;
      item.y = dy < 0 ? y + dy : y;
      item.w = Math.max(0.2, Math.abs(dx));
      item.h = kind === 'line' ? 0.35 : Math.max(0.2, Math.abs(dy));
...
function safeReplaceRect(X, Y, width, height, W, H) {
  const padX = Math.max(1.2, Math.min(2.6, width * 0.04));
  const padY = Math.max(1.8, Math.min(3.2, height * 0.24));
  const x = Math.max(0, X - padX);
  const y = Math.max(0, Y - padY);
  const right = Math.min(W, X + width + padX);
  const top = Math.min(H, Y + height + padY);
  return { x, y, width: Math.max(0.1, right - x), height: Math.max(0.1, top - y) };
}

async function downloadPdf() {
  if (!sourceBytes) return;
  try {
    toast('Creo il PDF modificato…');
    ...
    for (const spec of pages) {
      ...
      for (const item of spec.items) {
        ...
        if (item.kind === 'white') page.drawRectangle({ x: X, y: Y, width, height, color: rgbFromHex(item.background || '#ffffff') });
        if (item.kind === 'replace') page.drawRectangle({ ...safeReplaceRect(X, Y, width, height, W, H), color: rgbFromHex(item.background || '#ffffff') });
    ...
    anchor.href = url;
    anchor.download = `${baseName}-modificato.pdf`;
    document.body.appendChild(anchor);
    anchor.click();
    anchor.remove();
    setTimeout(() => URL.revokeObjectURL(url), 4000);
    toast('PDF salvato con copertura completa del testo sostituito.');
  } catch (error) { console.error(error); toast('Errore nell’esportazione. Prova con un PDF non protetto.'); }
}

window.PDFEditorAPI = {
  replaceScreenArea,
  isLoaded: () => Boolean(pdf && canvas.width),
  clearSelection: () => { selectedId = ''; render(); }
};
...
$('#duplicate').onclick = () => { snapshot(); const copy = deep(pageSpec()); copy.items.forEach((item) => (item.id = makeId())); pages.splice(current + 1, 0, copy); current += 1; render(); };
